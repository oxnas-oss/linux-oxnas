...
static void raid1_hw_raidable(mddev_t *mddev)
{
...
#if defined(CONFIG_SCSI_OX800SATA) || defined(CONFIG_SCSI_OX810SATA)
    ...
    if (!conf->mirrors[0].rdev->bdev->bd_disk ||
		...
		(oxnassata_get_port_no(conf->mirrors[0].rdev->bdev->bd_disk->queue) < 0)) {
    ...
    if (!conf->mirrors[1].rdev->bdev->bd_disk ||
		...
		(oxnassata_get_port_no(conf->mirrors[1].rdev->bdev->bd_disk->queue) < 0)) {
