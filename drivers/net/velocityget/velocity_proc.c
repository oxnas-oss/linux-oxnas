...
{"Dump registers", VELOCITY_PROC_FILE, FunConfRead, FunConfWrite, CONF_DUMP_REGS,    NULL, 0},
    ...
    case CONF_DUMP_REGS:
        {
            ...
            for (i=0; i <= MAC_REG_BYTEMSK3_3; ++i) {
                printk("0x%02x\t", CSR_READ_1(&pInfo->hw, i));
                if (!((i+1) % 8)) {
                    printk("\n0x%02x:\t", i+1);
                }
            }
            len=sprintf(buf,"Registers dumped to message log");
        }
        break;
    }
    return len;
