...
#ifdef GPIO_N0204_DISK1_DETECT
#if (GPIO_N0204_DISK1_DETECT < 32) && (GPIO_N0204_DISK2_DETECT < 32)
#define DISK1_ACT_LED       GPIO_N0204_DISK1_ACT_LED
#define DISK2_ACT_LED       GPIO_N0204_DISK2_ACT_LED
#define DISK1_DETECT        GPIO_N0204_DISK1_DETECT
#define DISK2_DETECT        GPIO_N0204_DISK2_DETECT
#define DISK1_MASK          (1UL << (GPIO_N0204_DISK1_DETECT))
#define DISK2_MASK          (1UL << (GPIO_N0204_DISK2_DETECT))
#define SWITCH_MASK         ((DISK1_MASK) | (DISK2_MASK))
...
#define FALLING_EDGE_DETECT GPIO_A_FALLING_EDGE_DETECT
#define RISING_EDGE_DETECT  GPIO_A_RISING_EDGE_DETECT
#define RISING_INT_REG      GPIO_A_RISING_EDGE_ACTIVE_HIGH_ENABLE
#define DATA_REG            GPIO_A_DATA
#else
#error "please modify it for you."
#endif
#endif

#ifdef GPIO_N2200_DISK1_DETECT
#if (GPIO_N2200_DISK1_DETECT < 32) && (GPIO_N2200_DISK2_DETECT < 32)
#define DISK1_ACT_LED       GPIO_N2200_DISK1_ACT_LED
#define DISK2_ACT_LED       GPIO_N2200_DISK2_ACT_LED
#define DISK1_DETECT        GPIO_N2200_DISK1_DETECT
#define DISK2_DETECT        GPIO_N2200_DISK2_DETECT
#define DISK1_MASK          (1UL << (GPIO_N2200_DISK1_DETECT))
#define DISK2_MASK          (1UL << (GPIO_N2200_DISK2_DETECT))
#define SWITCH_MASK         ((DISK1_MASK) | (DISK2_MASK))
#define IRQ_NUM             GPIO_1_INTERRUPT
#define INT_STATUS_REG      GPIO_A_INTERRUPT_STATUS_REGISTER
#define SWITCH_PRISEL_REG   SYS_CTRL_GPIO_PRIMSEL_CTRL_0
 ...
 */
typedef struct hdd_detect_driver_s {
        /** sysfs dir tree root for disk detect driver */
        struct kset kset;
        struct kobject hdd_detect;
} hdd_detect_driver_t;

static hdd_detect_driver_t hdd_detect_driver;

static void work_handler(struct work_struct * not_used) {
        kobject_uevent(&hdd_detect_driver.hdd_detect, KOBJ_OFFLINE);
}

DECLARE_WORK(hdd_detect_hotplug_work, work_handler);

static int port0 = -1;
static int port1 = -1;

static void PHYRDY_change(int porti)
{
    u32 tmp = 0;
    u32 bit0;
    u32 bit1;
    ...
    i=porti;
    ap=ox810sata_driver.ap[i];

    if(ap) {
        ox810sata_scr_read(ap, SCR_ERROR, &tmp);
//        printk("SCR_ERROR: %X\n",tmp);
//        if (tmp & N_Bit) { //bit 16

            ox810sata_scr_read(ap, SCR_STATUS, &tmp );
            bit0 = tmp & 0x1;
            bit1 = (tmp & 0x2) >> 1;
            if( bit0 && bit1 ) { // plugged in
                struct ata_link *link = &ap->link;
                struct ata_eh_info *ehi = &link->eh_info;
                printk("port %d plugged in \n",i);
                link->flags |= ATA_LFLAG_HRST_TO_RESUME;
                ata_ehi_hotplugged(ehi);
                ata_port_freeze(ap);
            }
            if( !(bit0 & bit1) ){ // unplugged
                struct ata_link *link = &ap->link;
                struct ata_eh_info *ehi = &link->eh_info;
                printk("port %d unplugged \n",i);
                ata_ehi_hotplugged(ehi);
                ata_port_abort(ap);
            }
            ox810sata_scr_read(ap, SCR_ERROR, &tmp);
            tmp |= N_Bit;
            ox810sata_scr_write(ap, SCR_ERROR, tmp);
//        } else printk("N_Bit is 0\n");
    } else {
        printk("ox810sata_driver.ap[%d] is NULL\n",i);
    }
}
 ...
 */
static irqreturn_t sata_plug_int_handler(int irq, void* dev_id)
{
    int status = IRQ_NONE;
    ...
    if (int_status & SWITCH_MASK) {
        unsigned int rising_status = readl((volatile unsigned long *)RISING_EDGE_DETECT);
        unsigned int falling_status = readl((volatile unsigned long *)FALLING_EDGE_DETECT);

        if (falling_status & DISK1_MASK) {
            printk("HDD1 plug\n");
            // Disable the DISK1 GPIO line falling interrupt, enable rising interrupt
            spin_lock(&oxnas_gpio_spinlock);
            writel(readl(FALLING_INT_REG) & ~DISK1_MASK, FALLING_INT_REG);
            writel(readl(FALLING_EDGE_DETECT) | DISK1_MASK, FALLING_EDGE_DETECT);
            writel(readl(RISING_INT_REG) | DISK1_MASK, RISING_INT_REG);
            timer.data = 0;
            spin_unlock(&oxnas_gpio_spinlock);
        }
	if (rising_status & DISK1_MASK) {
            printk("HDD1 remove\n");
            // Disable the DISK1 GPIO line rising interrupt, enable falling interrupt
            spin_lock(&oxnas_gpio_spinlock);
            writel(readl(RISING_INT_REG) & ~DISK1_MASK, RISING_INT_REG);
            writel(readl(RISING_EDGE_DETECT) | DISK1_MASK, RISING_EDGE_DETECT);
            writel(readl(FALLING_INT_REG) | DISK1_MASK, FALLING_INT_REG);
            timer.data = 0;
            spin_unlock(&oxnas_gpio_spinlock);
        }

        if (falling_status & DISK2_MASK) {
            printk("HDD2 plug\n");
            // Disable the DISK2 GPIO line falling interrupt, enable rising interrupt
            spin_lock(&oxnas_gpio_spinlock);
            writel(readl(FALLING_INT_REG) & ~DISK2_MASK, FALLING_INT_REG);
            writel(readl(FALLING_EDGE_DETECT) | DISK2_MASK, FALLING_EDGE_DETECT);
            writel(readl(RISING_INT_REG) | DISK2_MASK, RISING_INT_REG);
            timer.data = 1;
            spin_unlock(&oxnas_gpio_spinlock);
        }
	if (rising_status & DISK2_MASK) {
            printk("HDD2 remove\n");
            // Disable the DISK1 GPIO line rising interrupt, enable falling interrupt
            spin_lock(&oxnas_gpio_spinlock);
            writel(readl(RISING_INT_REG) & ~DISK2_MASK, RISING_INT_REG);
            writel(readl(RISING_EDGE_DETECT) | DISK2_MASK, RISING_EDGE_DETECT);
            writel(readl(FALLING_INT_REG) | DISK2_MASK, FALLING_INT_REG);
            timer.data = 1;
            spin_unlock(&oxnas_gpio_spinlock);
        }
	...
	if (timer.data == 0){
        ...
        if (printk_ratelimit())
            printk("INT_STATUS_REG: %X\n",readl((volatile unsigned long *)INT_STATUS_REG));
...
{
    if((data>=0) && (data<=1)) PHYRDY_change(data);
    else printk("some error %s\n",__FUNCTION__);
...
#ifdef DISK1_ACT_LED
    /* enable output */
    ox810_gpio_set_output(DISK1_ACT_LED);
    ox810_gpio_set_output(DISK2_ACT_LED);

    /* disk light off */
    ox810_gpio_write_bit(DISK1_ACT_LED,0);
    ox810_gpio_write_bit(DISK2_ACT_LED,0);
#endif  /* DISK1_ACT_LED */
...
#ifdef DISK1_ACT_LED
    /* disk light on */
    if(ap->scsi_host->host_no==0)
        ox810_gpio_write_bit(DISK1_ACT_LED,1);
    else if(ap->scsi_host->host_no==1)
        ox810_gpio_write_bit(DISK2_ACT_LED,1);
#endif  // DISK1_ACT_LED
...
static void ox810sata_postreset(struct ata_link *link, unsigned int *classes)
{
    ...
    if (ap==ox810sata_driver.ap[0]) {
        printk("port0 %s\n",ata_link_offline(link) ? "offline" : "online");
        port0=ata_link_offline(link) ? 0 : 1;
    }
    if (ap==ox810sata_driver.ap[1]) {
        printk("port1 %s\n",ata_link_offline(link) ? "offline" : "online");
        port1=ata_link_offline(link) ? 0 : 1;
    }
...
static int __init ox810sata_device_init( void )
{
    ...
    /* Install a shared interrupt handler on the appropriate GPIO bank's
       interrupt line */
    if (request_irq(IRQ_NUM, sata_plug_int_handler, IRQF_SHARED, "Disk Detect", &hdd_detect_driver)) {
            printk(KERN_ERR "Disk Detect: cannot register IRQ %d\n", IRQ_NUM);
            del_timer_sync(&timer);
            return -EIO;
    }

    ox810_gpio_set_input(DISK1_DETECT);
    ox810_gpio_set_input(DISK2_DETECT);
    disk1=ox810_gpio_read_bit(DISK1_DETECT);
    disk2=ox810_gpio_read_bit(DISK2_DETECT);
    ...
    /* Set up the disk detect GPIO line for active low, debounced interrupt */
    writel(readl(DEBOUNCE_REG)    | SWITCH_MASK, DEBOUNCE_REG);
    writel(readl(LEVEL_INT_REG)   | SWITCH_MASK, LEVEL_INT_REG);

    if (disk1)
        writel(readl(FALLING_INT_REG) | DISK1_MASK, FALLING_INT_REG);
    else writel(readl(RISING_INT_REG) | DISK1_MASK, RISING_INT_REG);

    if (disk2)
        writel(readl(FALLING_INT_REG) | DISK2_MASK, FALLING_INT_REG);
    else writel(readl(RISING_INT_REG) | DISK2_MASK, RISING_INT_REG);

    writel(readl(FALLING_EDGE_DETECT) | SWITCH_MASK, FALLING_EDGE_DETECT);
    writel(readl(RISING_EDGE_DETECT) | SWITCH_MASK, RISING_EDGE_DETECT);
    ...
    printk(KERN_INFO "Thecus SATA Disk Detect driver registered\n");
...
static void __exit ox810sata_device_exit(void)
{
    ...
    /* Disable interrupt generation by the disk detect GPIO line */
    spin_lock_irqsave(&oxnas_gpio_spinlock, flags);
    ...
    free_irq(IRQ_NUM, &hdd_detect_driver);
