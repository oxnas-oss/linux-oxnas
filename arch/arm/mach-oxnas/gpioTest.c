...
#define GPIO_TEST_SCL (0x00000001 << I2C_GPIO_SCL)
#define GPIO_TEST_SDA (0x00000001 << I2C_GPIO_SDA)
#define GPIO_TEST_SCS (0x00000001 << I2C_GPIO_SCS)
...
irqreturn_t irqHandler( int irq, void* dev_id, struct pt_regs* regs)
{
	...
	if ( !(temp & (GPIO_TEST_SCL | GPIO_TEST_SDA | GPIO_TEST_SCS) ) )
	{
		printk("Not for us...\n");
		return IRQ_NONE;
	}	
	...
	if ( test_bit( I2C_GPIO_SCL, (volatile unsigned long *) &temp ) )
	{
		printk("Int Found I2C_GPIO_SCL\n");
		*( (volatile unsigned long *) GPIO_A_RISING_EDGE_ACTIVE_HIGH_ENABLE ) &= ~(1UL <<  I2C_GPIO_SCL);
		*( (volatile unsigned long *) GPIO_A_FALLING_EDGE_ACTIVE_LOW_ENABLE ) &= ~(1UL <<  I2C_GPIO_SCL);
		*( (volatile unsigned long *) GPIO_A_LEVEL_INTERRUPT_ENABLE ) &= ~(1UL <<  I2C_GPIO_SCL);
		*( (volatile unsigned long *) GPIO_A_RISING_EDGE_DETECT ) &= ~(1UL <<  I2C_GPIO_SCL);
		*( (volatile unsigned long *) GPIO_A_FALLING_EDGE_DETECT ) &= ~(1UL <<  I2C_GPIO_SCL);
		pGPIO->found |= GPIO_TEST_SCL;
	}
	
	if ( test_bit( I2C_GPIO_SDA, (volatile unsigned long *) &temp ) )
	{
		printk("Int Found I2C_GPIO_SDA\n");
		*( (volatile unsigned long *) GPIO_A_RISING_EDGE_ACTIVE_HIGH_ENABLE ) &= ~(1UL <<  I2C_GPIO_SDA);
		*( (volatile unsigned long *) GPIO_A_FALLING_EDGE_ACTIVE_LOW_ENABLE ) &= ~(1UL <<  I2C_GPIO_SDA);
		*( (volatile unsigned long *) GPIO_A_LEVEL_INTERRUPT_ENABLE ) &= ~(1UL <<  I2C_GPIO_SDA);
		*( (volatile unsigned long *) GPIO_A_RISING_EDGE_DETECT ) &= ~(1UL <<  I2C_GPIO_SDA);
		*( (volatile unsigned long *) GPIO_A_FALLING_EDGE_DETECT ) &= ~(1UL <<  I2C_GPIO_SDA);
		pGPIO->found |= GPIO_TEST_SDA;
	}
	
	if ( test_bit( I2C_GPIO_SCS, (volatile unsigned long *) &temp ) )
	{
		printk("Int Found I2C_GPIO_SCS\n");
		*( (volatile unsigned long *) GPIO_A_RISING_EDGE_ACTIVE_HIGH_ENABLE ) &= ~(1UL <<  I2C_GPIO_SCS);
		*( (volatile unsigned long *) GPIO_A_FALLING_EDGE_ACTIVE_LOW_ENABLE ) &= ~(1UL <<  I2C_GPIO_SCS);
		*( (volatile unsigned long *) GPIO_A_LEVEL_INTERRUPT_ENABLE ) &= ~(1UL <<  I2C_GPIO_SCS);
		*( (volatile unsigned long *) GPIO_A_RISING_EDGE_DETECT ) &= ~(1UL <<  I2C_GPIO_SCS);
		*( (volatile unsigned long *) GPIO_A_FALLING_EDGE_DETECT ) &= ~(1UL <<  I2C_GPIO_SCS);
		pGPIO->found |= GPIO_TEST_SCS;
	}
...
int gpioTest_go(void)
{
	...
	//assert( I2C_GPIO_SDA < I2C_GPIO_SCL );
	//assert( I2C_GPIO_SCL < I2C_GPIO_SCS );
	...
	*( (volatile unsigned long *) SYS_CTRL_GPIO_PRIMSEL_CTRL_0 ) &= ~(1UL <<  (I2C_GPIO_SDA  & 0x0000001F) );
	*( (volatile unsigned long *) SYS_CTRL_GPIO_PRIMSEL_CTRL_0 ) &= ~(1UL <<  (I2C_GPIO_SCL  & 0x0000001F) );
	*( (volatile unsigned long *) SYS_CTRL_GPIO_PRIMSEL_CTRL_0 ) &= ~(1UL <<  (I2C_GPIO_SCS  & 0x0000001F) );
	                                                                     
	*( (volatile unsigned long *) SYS_CTRL_GPIO_SECSEL_CTRL_0 ) &= ~(1UL <<  (I2C_GPIO_SDA  & 0x0000001F) );	
	*( (volatile unsigned long *) SYS_CTRL_GPIO_SECSEL_CTRL_0 ) &= ~(1UL <<  (I2C_GPIO_SCL  & 0x0000001F) );	
	*( (volatile unsigned long *) SYS_CTRL_GPIO_SECSEL_CTRL_0 ) &= ~(1UL <<  (I2C_GPIO_SCS  & 0x0000001F) );	
...
for (i=0; i<65000; ++i)
{
	temp = (i & 0x00000007) << I2C_GPIO_SCL;
...
DumpGPIO();	
	...
	writel( temp, GPIO_A_OUTPUT_ENABLE );
	udelay(1);
	if ( *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SDA) ||
	     *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SCL) ||
	     *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SCS) )
	{
	...
	if ( !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SDA) ) ||
	     !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SCL) ) /*||
	     !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SCS) ) */ )
	{
	...
	*( (volatile unsigned long *) GPIO_A_OUTPUT_CLEAR   ) |= (1UL <<  (I2C_GPIO_SDA  & 0x0000001F) );	
	*( (volatile unsigned long *) GPIO_A_OUTPUT_CLEAR   ) |= (1UL <<  (I2C_GPIO_SCL  & 0x0000001F) );	
	*( (volatile unsigned long *) GPIO_A_OUTPUT_CLEAR   ) |= (1UL <<  (I2C_GPIO_SCS  & 0x0000001F) );	
	for (i=I2C_GPIO_SCL; i<=/*I2C_GPIO_SCS*/I2C_GPIO_SDA; ++i)
	{
		...
		if ( ( *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SDA) ) ||
		     ( *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SCL) ) ||
		     ( *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SCS) ) ||
		     !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SDA) )         ||
		     !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SCL) )   /*      ||
		     !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SCS) ) */        )
		{
		...
		*( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE_CLEAR ) |= (1UL <<  i);		
		...
		if ( temp != readl( GPIO_A_OUTPUT_ENABLE )        ||
		     ( *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SDA) ) ||
		     ( *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SCL) ) ||
		     ( *( (volatile unsigned long *) GPIO_A_OUTPUT_ENABLE) & (1UL << I2C_GPIO_SCS) ) ||
		     !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SDA) )         ||
		     !( *( (volatile unsigned long *) GPIO_A_DATA) & (1UL << I2C_GPIO_SCL) )    /*     ||
	...
	request_irq( 
		...
		SA_SHIRQ,                	// unsigned long 	irq_flags,
	...
	{
		int tmo;
		int drv = i+1 > I2C_GPIO_SCS ? I2C_GPIO_SCL : i+1;
...
static int gpioTest_init(void){
	...
	res  = readl( GPIO_A_INPUT_DEBOUNCE_ENABLE );
		res &= ~(GPIO_TEST_SCL | GPIO_TEST_SDA | GPIO_TEST_SCS);
