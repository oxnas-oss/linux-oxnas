		  ...
		  int count, int *eof, void *unused)
{
	...
	for (i=0; i < 3; i++)
	{
		len += sprintf(buf+len, "usb port %d %p:%lx \n", i, (usbport+4*i) , *(usbport+i));
	}
	*eof=1;
	return len;
}
