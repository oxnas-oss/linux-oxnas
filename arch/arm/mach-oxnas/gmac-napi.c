...
static void initialise_phy(gmac_priv_t* priv)
{
	switch (priv->phy_type) {
		...
		case PHY_TYPE_LSI_ET1011C:
			{
				u32 config_reg = priv->mii.mdio_read(priv->netdev, priv->mii.phy_id, ET1011C_MII_CONFIG);

				printk(KERN_INFO "%s: PHY is LSI ET1011C\n", priv->netdev->name);
				config_reg &= ~(((1UL << ET1011C_MII_CONFIG_IFMODESEL_NUM_BITS) - 1) << ET1011C_MII_CONFIG_IFMODESEL);
				config_reg |= (ET1011C_MII_CONFIG_IFMODESEL_GMII_MII << ET1011C_MII_CONFIG_IFMODESEL);
				config_reg |= ((1UL << ET1011C_MII_CONFIG_SYSCLKEN) |
                              (1UL << ET1011C_MII_CONFIG_TXCLKEN) |
							   (1UL << ET1011C_MII_CONFIG_TBI_RATESEL) |
							   (1UL << ET1011C_MII_CONFIG_CRS_TX_EN));
				priv->mii.mdio_write(priv->netdev, priv->mii.phy_id, ET1011C_MII_CONFIG, config_reg);
			}
			break;
	}
}
...
static void watchdog_timer_action(unsigned long arg)
{
	...
	if (priv->phy_force_negotiation || (state == WDS_RESETTING)) {
		...
		duplex_changed = mii_check_media_ex(&priv->mii, 1, priv->mii_init_media, &gigabit_changed, &pause_changed);
...
static int process_rx_packet(gmac_priv_t *priv)
{
	...
	while (1) {
		...
		if (!header) {
			// Remember where the packet IP header is
			header = packet;
		...
		// Fragment's page is now referenced by fragment metadata
		frag_info.page = 0;
		...
		if (last) {
			...
			if (likely(valid)) {
				...
				// 802.1Q VLAN tag presence
				int vlan_offset = 0;
				unsigned short eth_protocol = ntohs(((struct ethhdr*)header)->h_proto);
				int is_ip = is_ip_packet(eth_protocol);
				...
				if (!is_ip) {
					...
					if (eth_protocol == ETH_P_8021Q) {
						// Extract the contained protocol type from after
						// the VLAN tag
						eth_protocol = ntohs(*(unsigned short*)(header + ETH_HLEN));
						is_ip = is_ip_packet(eth_protocol);
...
#endif // !CONFIG_OXNAS_VERSION_0X800
					if (is_ipv4_packet(eth_protocol)) {
						ipv4_header = (struct iphdr*)(header + ETH_HLEN + vlan_offset);
...
#ifdef SUPPORT_IPV6
						struct ipv6hdr* ipv6_header = (struct ipv6hdr*)(header + ETH_HLEN + vlan_offset);
...
not_valid:
	// If the fragment's page was not accounted for by the fragment metadata
	if (frag_info.page) {
		put_page(frag_info.page);
	}
	...
	int               *budget)
{
    ...
    do {
        ...
        if (rx_work_limit) {
            ...
            continue_polling = rx_available_for_read(&priv->rx_gmac_desc_list_info, 0);
    ...
    struct sk_buff             *skb)
{
    ...
    if (nr_frags > COPRO_NUM_TX_FRAGS_DIRECT) {
        ...
        err = skb_linearize(skb, GFP_KERNEL);
        if (err) {
    ...
    if (unlikely(netif_queue_stopped(dev))) {
        ...
        if (priv->tx_pending_skb) {
            ...
            if (set_tx_descriptor(priv,
                                  ...
                                  priv->tx_pending_skb->ip_summed == CHECKSUM_HW) >= 0) {
